...
\item Could try to add user options to different background pictures later on;
\end{itemize}
\subsubsection{start newBtn}
\label{sec-5-2-2}
\begin{itemize}
\item Realized becuase I didn't use Bitmap during last checkin, my newBtn lost its functionality, and I fixed it by writing to Bitmap;
\item The wired drawing path shapes got Corrected as well by writing to Bitmap;
...
\item I googled the answers that
\begin{itemize}
...
\item I have \textbf{difficulty} to solve this one;
\end{itemize}
\end{itemize}
\subsubsection{Undo/Redo}
\label{sec-5-2-3}
\begin{itemize}
\item Still got confused why I will have to wrap my ListView items into different objects (because I want undo/redo not only Path, but also \textbf{RectF} etc?)
\item How do I enum \textbf{Objects} instead of Java data types
