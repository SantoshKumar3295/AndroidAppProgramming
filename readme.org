...
- Could try to add user options to different background pictures later on;
*** start newBtn
- Realized becuase I didn't use Bitmap during last checkin, my newBtn lost its functionality, and I fixed it by writing to Bitmap;
- The wired drawing path shapes got Corrected as well by writing to Bitmap;
...
- I googled the answers that
  ...
  - I have *difficulty* to solve this one;
*** Undo/Redo
- Still got confused why I will have to wrap my ListView items into different objects (because I want undo/redo not only Path, but also *RectF* etc?)
- How do I enum *Objects* instead of Java data types
