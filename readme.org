...
*** openBtn for loading an image file as an ImageView
  ...
  - *Question*: Right now, my image is an ImageView in my layout, what ideas that I could use to set/change/transfer my ImageView to be my draw view background? use Bitmap
  - References:
...
*** Undo/Redo
  ...
  - Implemented by packing a wrapper class myPath to wrap the Path(), drawPaint color, and drawPaint strokesize together as an object. 
  ...
  - *Questions*: 
    ...
    2. One little detail though, I dras after touch up, my paint color change delayed, how do I implement *real time*?
       ...
       *Answered*: drawPath.reset() produced all the trouble. Later on, I need to think in a debuggin way/manner. 
